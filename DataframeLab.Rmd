...
```{r}
(USPE_data <- data.frame(USPersonalExpenditure))
```
...
```{r}
USPE_data$Category <- row.names(USPE_data)
USPE_data
```
...
```{r}
(tidy_uspe <- pivot_longer(USPE_data, c("X1940","X1945", "X1950","X1955" ,"X1960"), names_to = 'year', values_to = 'expendture'))
```


How much money was spent on personal care in 1940?
```{r}
tidy_uspe$expendture[tidy_uspe$year=='X1940' & tidy_uspe$Category=="Personal Care"] 
```


How much money was spent on Food and Tobacco in 1960?
```{r}
tidy_uspe$expendture[tidy_uspe$year=='X1960' & tidy_uspe$Category=="Food and Tobacco"] 
...
```{r}
tidy_uspe$Category[tidy_uspe$expendture == max(tidy_uspe$expendture[tidy_uspe$year=='X1960'])] 
```
...
lowest_category <- function(years){
  tidy_uspe$Category[tidy_uspe$expendture==min(tidy_uspe$expendture[tidy_uspe$year== years])]
  
}
lowest_category('X1950')
```
...
```{r}
sapply(tidy_uspe$year, lowest_category)
```
